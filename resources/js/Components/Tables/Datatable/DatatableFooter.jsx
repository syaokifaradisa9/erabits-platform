        ...
        <div className="flex flex-col items-center justify-between gap-4 mt-4 md:flex-row">
            ...
            >
                {dataTable.links?.map((link, index, array) => {
                    // Add this check to ensure link and link.url are not null/undefined
                    if (!link || !link.url) {
                        return null; // Or render a disabled link/placeholder if needed
                    }
                    ...
                    return (
                        <Link
                            key={`page-${index}`}
                            href={link.url}
                            onClick={link.url ? onChangePage : undefined}
                            className={`
